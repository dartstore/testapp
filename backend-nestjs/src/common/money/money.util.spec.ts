...
describe('arithmetic', () => {
  it('adds and subtracts', () => {
    ...
    expect(subtract(money(1050n, 'USD'), money(50n, 'USD')).amountMinor).toBe(1000n)
  })
  ...
  it('sums a list', () => {
    expect(
      sum([money(100n, 'USD'), money(250n, 'USD'), money(-50n, 'USD')]).amountMinor,
    ).toBe(300n)
  })
...
describe('allocate', () => {
  it('splits evenly when it divides cleanly', () => {
    expect(allocateEvenly(money(300n, 'USD'), 3).map((m) => m.amountMinor)).toEqual([
      100n,
      100n,
      100n,
    ])
  ...
  it('distributes the remainder to the earliest largest remainders', () => {
    expect(allocateEvenly(money(100n, 'USD'), 3).map((m) => m.amountMinor)).toEqual([
      34n,
      33n,
      33n,
    ])
  ...
  it('respects weights', () => {
    expect(
      allocate(money(1000n, 'USD'), [70n, 30n]).map((m) => m.amountMinor),
    ).toEqual([700n, 300n])
  })
  ...
  it('supports a zero weight beneficiary', () => {
    expect(
      allocate(money(100n, 'USD'), [1n, 0n]).map((m) => m.amountMinor),
    ).toEqual([100n, 0n])
  })
  ...
  it('preserves the currency', () => {
    expect(allocateEvenly(money(100n, 'KWD'), 3).every((m) => m.currency === 'KWD')).toBe(
      true,
    )
  })
})
  ...
  it('allocation is deterministic', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.bigInt({ min: 1n, max: 1000n }), { minLength: 1, maxLength: 8 }),
        (amountMinor, weights) => {
...
describe('gateway and transport helpers', () => {
  it('serializes and deserializes losslessly', () => {
    ...
    expect(deserialize(serialize(original)).amountMinor).toBe(original.amountMinor)
  })
...
describe('format', () => {
  it('groups thousands and appends the symbol', () => {
    ...
    expect(format(money(123456789n, 'USD'), { withSymbol: false })).toBe('1,234,567.89')
  })

  it('formats zero-decimal and three-decimal currencies', () => {
    expect(format(money(1000n, 'JPY'), { withSymbol: false })).toBe('1,000')
    expect(format(money(1234567n, 'KWD'), { withSymbol: false })).toBe('1,234.567')
  })
