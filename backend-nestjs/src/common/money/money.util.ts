...
export function money(amountMinor: bigint | number, currency: string): Money {
  ...
  if (typeof amountMinor === 'number') {
    ...
    return Object.freeze({ amountMinor: BigInt(amountMinor), currency: normalized })
  }
...
export function parseDecimal(input: string, currency: string): Money {
  ...
  const digits = `${integerPart}${paddedFraction}`
...
export function toDecimalString(value: Money): string {
  ...
  return `${negative ? '-' : ''}${major}.${minor.toString().padStart(exponent, '0')}`
}
 ...
 */
export function multiplyByInteger(value: Money, factor: bigint | number): Money {
 ...
 */
export function allocate(value: Money, weights: readonly (bigint | number)[]): Money[] {
  if (weights.length === 0) {
  ...
  return shares.map((share) =>
    money(negative ? -share : share, value.currency),
  )
}
...
export function allocateEvenly(value: Money, parts: number): Money[] {
  if (!Number.isSafeInteger(parts) || parts < 1) {
    throw new MoneyError(`عدد الأجزاء لازم يكون عدد صحيح موجب (استلمنا: ${parts}).`)
  }
...
export function serialize(value: Money): { amountMinor: string; currency: CurrencyCode } {
  return { amountMinor: value.amountMinor.toString(), currency: value.currency }
}

export function deserialize(raw: { amountMinor: string; currency: string }): Money {
  if (!/^-?\d+$/.test(raw.amountMinor)) {
    throw new MoneyParseError(raw.amountMinor, 'المبلغ بالوحدات الصغرى لازم يكون عدد صحيح.')
  }
