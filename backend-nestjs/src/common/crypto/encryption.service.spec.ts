...
describe('EncryptionService', () => {
  describe('round-trip', () => {
    ...
    it('يتعامل مع النص الفاضي (F1)', async () => {
      const service = makeService()
  ...
  /* B3 — المفتاح الجذري مايتستخدمش كمفتاح تشفير (F6-corrected) */
  describe('platform key derivation (B3)', () => {
    function rebuildAad(context: EncryptionContext): Buffer {
      ...
      return Buffer.concat([
        prefixed('aad:v1'),
        prefixed(String(EnvelopeScope.Platform)),
        prefixed(''),
        prefixed(context.mode),
        prefixed(context.recordType),
        prefixed(context.recordId),
        prefixed(context.field),
      ])
    }

    function tryDecryptWith(key: Buffer, envelope: Buffer): string {
      const iv = envelope.subarray(6, 18)
      const tag = envelope.subarray(18, 34)
      const ciphertext = envelope.subarray(34)

      const decipher = crypto.createDecipheriv('aes-256-gcm', key, iv)
      decipher.setAAD(rebuildAad(CTX))
    ...
    it('مفتاح المنصة المشتق بيفك النص — يعني الـ AAD صح', async () => {
      const envelope = Buffer.from(
        await makeService().encrypt('probe', CTX),
        'base64',
      )
      ...
      const derived = Buffer.from(
        crypto.hkdfSync(
          ...
          Buffer.from(`platform:v${PLATFORM_KEY_VERSION}`, 'utf8'),
          32,
        ),
      )

      expect(tryDecryptWith(derived, envelope)).toBe('probe')
    })

    it('المفتاح الجذري نفسه مابيفكش النص — بنفس الـ AAD بالظبط', async () => {
      const envelope = Buffer.from(
        await makeService().encrypt('probe', CTX),
        'base64',
      )

      expect(() => tryDecryptWith(KEK_V1, envelope)).toThrow()
    })
  ...
  describe('AAD binding (B1)', () => {
    ...
    /* F2 — خطأ السياق مش حادث أمني */
    it('خطأ السياق بيطلع كخطأ برمجي مش كانتهاك سلامة (F2)', async () => {
      const service = makeService()
      const payload = await service.encrypt('secret', CTX)

      await expect(
        service.decrypt(payload, { ...CTX, recordId: '' }),
      ).rejects.not.toBeInstanceOf(DecryptionError)
